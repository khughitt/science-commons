        ...
        resources.append(
            {
                ...
                "source": {
                    "type": "local",
                    "ref": f"{OUTPUT_ROOT_TOKEN}/go/{filename}",
                },
                "hash": sha256,
                "bytes": byte_count,
            }
        )
    return {"name": "go", "resources": resources}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render GO datapackage.yaml.")
    parser.add_argument("--data-dir", type=Path)
    parser.add_argument("--output-path", type=Path)
    args = parser.parse_args()
    data_dir = args.data_dir or resolve_commons_data_root() / "go"
